...
export function NetworkSettingsModal({ open, onClose }: NetworkSettingsModalProps) {
  ...
  const save = async () => {
    ...
    setError(null);
    const clientOnly = Boolean(status?.isClientOnlyBuild);
    if (clientOnly) {
      if (!remoteServerUrl.trim()) {
        setError("Укажите адрес удалённого хоста (URL или IP с портом при необходимости).");
        return;
      }
      if (!apiKey.trim()) {
        setError("Укажите API-ключ — без него сервер отклонит запросы.");
        return;
      }
    }
    try {
      const res = await window.electronAPI.setNetworkSettings({
        serveAsHost: clientOnly ? false : serveAsHost,
    ...
    >
      ...
      >
        ...
        <p className="mt-2 text-sm leading-relaxed text-zinc-400">
          {clientOnly
            ? "Клиентская сборка: локальная SQLite и генераторы на этой машине отключены. Работа только через ваш удалённый хост — нужны URL и API-ключ."
            : "Один .exe может быть сервером с вашей SQLite или клиентом к копии друга. Задайте общий секрет API-ключа на обеих сторонах."}
        </p>
          ...
          <>
            ...
            {lockedRemote ? (
              <p className="mt-4 rounded-lg border border-rose-500/45 bg-rose-950/50 px-3 py-3 text-[13px] leading-snug text-rose-50">
                <strong>{status?.clientRemoteRequiredMessage ?? "Подключитесь к серверу хоста"}.</strong>{" "}
                {clientOnly
                  ? "Поля «Remote Host» и «API Key» обязательны. Сохраните настройки, чтобы включить генерацию."
                  : "Укажите адрес и API-ключ ниже и нажмите «Сохранить». Без соединения генерация лута и NPC недоступна."}
              </p>
            ) : null}

            {serveAsHost && remoteServerUrl.trim() && !clientOnly ? (
            ...
            ) : !clientOnly && serveAsHost && apiKey.trim() ? (
            ...
            {!clientOnly ? (
              ...
              <p className="mt-6 rounded-lg border border-zinc-600/50 bg-zinc-900/40 px-3 py-2 text-[13px] text-zinc-400">
                Режим «сервер на этой машине» в клиентской сборке недоступен.
              </p>
            )}
            ...
            <label className="mt-4 block">
              <span className="text-[11px] uppercase tracking-[0.18em] text-zinc-500">
                Remote Host (URL / IP) {clientOnly ? <span className="text-rose-400">*</span> : null}
              </span>
              <input
                type="text"
                value={remoteServerUrl}
                onChange={(e) => setRemoteServerUrl(e.target.value)}
                placeholder="https://host.example:3000 или IP без схемы"
                className="mt-1 w-full rounded-xl border border-gold/20 bg-black/50 px-3 py-2 text-sm text-zinc-100 outline-none focus:border-gold/45"
                required={clientOnly}
              />
            ...
            <label className="mt-4 block">
              <span className="text-[11px] uppercase tracking-[0.18em] text-zinc-500">
                API Key {clientOnly ? <span className="text-rose-400">*</span> : null}
              </span>
              <input
                type="password"
                autoComplete="off"
                ...
                placeholder="Общий секрет с хостом (обязательно)"
                className="mt-1 w-full rounded-xl border border-gold/20 bg-black/50 px-3 py-2 text-sm text-zinc-100 outline-none focus:border-gold/45"
                required={clientOnly}
              />
              <span className="mt-1 block text-[12px] text-zinc-600">
                Заголовок <code className="text-zinc-400">X-Zernix-Api-Key</code> или Bearer.{" "}
                {clientOnly ? "Без ключа клиент не сможет вызывать API." : "Без ключа сервер не стартует."}
              </span>
            </label>
